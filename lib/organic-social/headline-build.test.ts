...
test('Overview builds exactly the five OVERVIEW_KPI_KEYS, in order', () => {
  const h = buildPlatformHeadline('TWITTER', full('TWITTER'), OVERVIEW_KPI_KEYS)
...
test('platform build includes the full per-channel KPI set, in order', () => {
  ...
  const h = buildPlatformHeadline('LINKEDIN', m, keys)
...
test('exposure label is Views for Instagram, Impressions for X', () => {
  const ig = buildPlatformHeadline('INSTAGRAM', full('INSTAGRAM'), OVERVIEW_KPI_KEYS)
  const x = buildPlatformHeadline('TWITTER', full('TWITTER'), OVERVIEW_KPI_KEYS)
...
test('percent KPIs carry format percent; number KPIs number', () => {
  const h = buildPlatformHeadline('INSTAGRAM', full('INSTAGRAM'), OVERVIEW_KPI_KEYS)
...
test('X uses Profile Clicks (not Views); Facebook omits Profile Views', () => {
  ...
  const xh = buildPlatformHeadline('TWITTER', metricsForKeys('TWITTER', x, 1), x)
...
test('Facebook engagements carries the decision-6 footnote', () => {
  const keys = platformKpiKeys('FACEBOOK')
  const h = buildPlatformHeadline('FACEBOOK', metricsForKeys('FACEBOOK', keys, 1), keys)
...
test('omitted requested metric still throws (guard preserved)', () => {
  ...
  expect(() => buildPlatformHeadline('LINKEDIN', m, keys)).toThrow(/omitted requested metric/)
})
...
test('names the omitted metric in the error', () => {
  ...
  delete m[engMetric]
  expect(() => buildPlatformHeadline('TWITTER', m, OVERVIEW_KPI_KEYS)).toThrow(engMetric)
})
...
test('a present value:null renders 0 rather than throwing', () => {
  ...
  const h = buildPlatformHeadline('INSTAGRAM', m, OVERVIEW_KPI_KEYS)
...
test('per-KPI delta computed from context; undefined when no context present', () => {
  ...
  const h = buildPlatformHeadline('TWITTER', withCtx, OVERVIEW_KPI_KEYS)
