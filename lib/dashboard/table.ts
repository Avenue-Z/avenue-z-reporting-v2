...
import type { GroupedRow, LabelOverrides, MetricFormat } from './types'
import { formatMetric } from './format'
import { resolveDimLabel, resolveValueLabel } from './labels'
...
export interface TableInput {
  columns: {
    ...
    /** Set on the dimension column so DataTable renders editable label cells/header. */
    editable?: boolean
    dimKey?: string
  }[]
...
export function toTableInput(
  ...
  const columns: TableInput['columns'] = [
    { key: dimKey, label: resolveDimLabel(overrides, dimKey), align: 'left', sortable: true, sortType: 'string', editable: true, dimKey },
  ...
  const rows = r.rows.map((row) => {
    const raw = row.dim[dimKey] ?? ''
    const out: Record<string, ReactNode> = {
      [dimKey]: resolveValueLabel(overrides, dimKey, raw),
      [`${dimKey}__raw`]: raw,
