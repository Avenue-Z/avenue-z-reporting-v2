...
export interface LabelOverrides {
  /** dimKey → { rawValue → display }. e.g. { channel: { 'facebook-ads': 'Facebook Ads' } } */
  values?: Record<string, Record<string, string>>
  /** dimKey → column-header display. e.g. { channel: 'Channel' } */
  dims?: Record<string, string>
}

/** Where an inline edit writes. `blockText` = block copy; `labelValue`/`labelDim`
 *  = dashboard-wide dimension label overrides. Serializable (crosses RSC boundary). */
export type EditTarget =
  | { kind: 'blockText'; blockId: string; field: 'name' | 'narrativeBody' }
  | { kind: 'labelValue'; dimKey: string; rawValue: string }
  | { kind: 'labelDim'; dimKey: string }
