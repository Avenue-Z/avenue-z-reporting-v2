import type {
  ...
  FormulaBinding, FormulaOperand, Granularity, LabelOverrides, LeafBinding, MetricFormat, PersistedBlock, ShopifyBinding,
...
function parseLabelOverrides(v: unknown, path: string): { ok: true; value: LabelOverrides | undefined } | { ok: false; error: string } {
  if (v === undefined) return { ok: true, value: undefined }
  if (!isObj(v)) return { ok: false, error: `${path}: expected object` }
  const out: LabelOverrides = {}
  if (v.values !== undefined) {
    if (!isObj(v.values)) return { ok: false, error: `${path}.values: expected object` }
    const values: Record<string, Record<string, string>> = {}
    for (const [dimKey, m] of Object.entries(v.values)) {
      if (!isObj(m)) return { ok: false, error: `${path}.values.${dimKey}: expected object` }
      const inner: Record<string, string> = {}
      for (const [raw, label] of Object.entries(m)) {
        if (!isStr(label)) return { ok: false, error: `${path}.values.${dimKey}.${raw}: expected string` }
        inner[raw] = label
      }
      values[dimKey] = inner
    }
  ...
  if (v.dims !== undefined) {
    if (!isObj(v.dims)) return { ok: false, error: `${path}.dims: expected object` }
    const dims: Record<string, string> = {}
    for (const [dimKey, label] of Object.entries(v.dims)) {
      if (!isStr(label)) return { ok: false, error: `${path}.dims.${dimKey}: expected string` }
      dims[dimKey] = label
    }
    out.dims = dims
  }
...
export function parseDashboardConfig(
  ...
  const lo = parseLabelOverrides(v.labelOverrides, 'config.labelOverrides')
  if (!lo.ok) return lo
  return { ok: true, config: { defaultRange: dr.value, blocks, ...(lo.value ? { labelOverrides: lo.value } : {}) } }
}
