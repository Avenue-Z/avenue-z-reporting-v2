...
describe('sanitizeCommentaryHtml', () => {
  ...
  test('drops javascript: hrefs but keeps http/https/mailto and adds safe rel/target', () => {
    const js = sanitizeCommentaryHtml('<a href="javascript:alert(1)">x</a>')
    expect(js).not.toContain('javascript:')  // disallowed scheme dropped
    expect(js).not.toContain('href')          // no href survives
    expect(js).toContain('x')                 // link text preserved
  ...
  test('neutralizes whitespace-obfuscated javascript: hrefs', () => {
    const out = sanitizeCommentaryHtml('<a href="  javascript:alert(1)">x</a>')
