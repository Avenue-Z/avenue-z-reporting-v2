...
Jasmine answered the decisions doc on 2026-09-21, and my two follow-ups the same evening:
...
| Outline fixes (Video Views, 5 top posts, collab rule, deck-basis post rate) | PR 255 (plan on the branch) |
| Lock every number | PR 256 (plan on the branch) |
| YTD Review | PR 255 (plan on the branch; needs nothing from 256) |

Update 2026-09-22: every PR in the set stands alone. None needs another to merge
first, none carries another's commits, and all merge with zero conflicts in any
order. So the outline fixes and YTD Review moved onto 255, lock every number onto
256, and the stacked drafts (257, 258, 259) are closed. The two new tables (the
locks and the annotation hides) ship in ONE migration commit that is merged into
both 256 and 252, so either can merge first and the migration runs once.
