...
|---|-----|--------|----------|---------|
| 1 | ● | RESOLVED | `content-types.ts` metric field maps | **GATE (C1) CLEARED — and it surfaced more than the rate.** Reconciled all four channels against Dash's per-post "– Organic" insights (2026-07-28). Fixes: **IG** engagements→`sum_total_engagements` (incl. reposts), rate→`engagement`, effectiveness→`effectiveness_engagements`×100 (was showing a nonsense ~1%); **FB** rate→`organic_engagement_rate_v2`, effectiveness→`organic_effectiveness_v2`. LI/X confirmed already correct. Every card metric now matches Dash. |
