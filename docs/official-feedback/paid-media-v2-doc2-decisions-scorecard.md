...
| 5 (default landing) | Paid Media opens on **Paid Search**, because it is the `{id: null}` entry in `PAID_MEDIA_SUBSECTIONS` (`lib/constants.ts:174`). **AEO and GA4 already do what item 5 asks**, with `{id: null, label: 'Overview'}` first. So the change is: make Overview `id: null` and give Paid Search its own id, then add the routing branch. The doc's claim that this matches AEO and GA4 is **verified true**. | `lib/constants.ts:174`, `app/dashboard/[clientSlug]/reports/page.tsx:73-75` |
...
| 11 / D2-B81 (cents) | **Root cause found, and the doc's observation is correct.** `usd()` is `'$' + Math.round(n)`, so it renders **whole dollars and never cents** (`lib/supermetrics/format.ts:1`). The KPI card uses it. **Meta's Top Regions chart charts `spend` but does not pass `valueFormat="currency"`** (`meta-ads/geo-section.tsx:52`), so Recharts prints the raw value with decimals. **LinkedIn's identical chart does pass it** (`linkedin-ads/geo-section.tsx:26`). That single missing prop is exactly the "chart shows cents, card shows whole dollars" mismatch Dianna flagged, and it is a Meta-vs-LinkedIn inconsistency. Note her instruction is "make them all with cents", which is the **opposite** direction from the existing formatter, so satisfying it means adding a cents-capable formatter rather than just passing the prop. | `lib/supermetrics/format.ts:1`, `components/report-sections/meta-ads/geo-section.tsx:52`, `components/report-sections/linkedin-ads/geo-section.tsx:26` |
