...
|---|---|---|
| Req 1 (Overview subpage) | **Does not exist, but the pattern to copy does.** Paid Media renders as one `section` with subsections (`?section=paid-media&subsection=meta\|linkedin`), driven by `PAID_MEDIA_SUBSECTIONS` in `lib/constants.ts:174`. Today that list is `[{id: null, label: 'Paid Search'}, {id: 'meta'}, {id: 'linkedin'}]`, so **Paid Search is the `id: null` default landing**. **AEO and GA4 already use exactly the shape Req 1 asks for**, with `{id: null, label: 'Overview'}` first (`AEO_SUBSECTIONS`, `lib/constants.ts:159`). So the nav and routing change is small and follows precedent. The genuinely new work is the **cross-channel aggregation layer**, which has no equivalent anywhere. | `lib/constants.ts:174`, `app/dashboard/[clientSlug]/reports/page.tsx:73`, `components/layout/sidebar.tsx:582`, `components/layout/portal-sidebar.tsx:239` |
