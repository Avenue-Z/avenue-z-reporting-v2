...
- **Decision (2026-08-04, Paul) — per-channel leads, no blended top line.** Confirmed Meta leads are a data gap. Rather than a blank blended metric or none at all: keep the blended **Leads / Cost-per-lead top line absent**, and show **per-channel Leads in the By-Channel breakdown** where available (Paid Search + LinkedIn populated; Meta renders `—`). Implemented in `lib/paid-media/overview.ts` (`ChannelMetrics.leads`, read via a per-channel `leadsKey`; Meta has none → `null`) and the Overview UI (Leads column + a caption stating Meta lead conversions are unavailable and there is no blended Leads total). No distortion: a blended number is never computed from partial channels.
- **Owner:** Dianna. Sign-off needed only to confirm the per-channel-leads display is acceptable and that blended Leads/CPL stays parked until Meta lead tracking exists (see `docs/official-feedback/paid-media-v2-leads-cpl-definition-question.md`). Does not block the Overview from shipping.
