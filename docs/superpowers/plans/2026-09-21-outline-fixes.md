...
| Instagram CONTENT posts carry the author at `instagram_user.handle` (or `.username`) and collaborators at `instagram.collaborators[].username` | `collab-posts-authors-2026-09-21.out` |
...
- Config is read at runtime from `dash_social_config` as `unknown` (no schema edit): `ownHandles` = `{ instagram?: string }`. Posts per page come from the part pin's threshold (`report_section_config.organic-social.thresholds['top-content']`), default 5, clamped 1 to 50.
...
1. **5 posts per page, paging kept.** D16 says 5 per tab; the card gallery pages today, so 5 per page keeps every post reachable. A hard cap at 5 is the alternative.
...
3. **Existing frozen Top Content windows on staging have no author**, so the author rule falls back to #ad for them until they are re-frozen. Clearing those staging rows is a staging write and waits for my go (and fits best with the lock-every-number build, which re-times Top Content freezing).
...
| `lib/organic-social/top-content.ts` | `fetchTopContent(..., opts?: { withAuthor?: boolean })`; the two `content.map(normalizePost)` lines attach the author only when asked |
| `lib/organic-social/outline-top-content.ts` (new) | `parseOwnHandles`, `suggestByAuthor`, `partitionByAuthor`, `withViewsBasisRate`, `postsPerPage` (pure) |
...
**Files:** Test: `lib/organic-social/outline-fixes-parity.test.ts` (new)

- [ ] **Step 1: Write the characterisation test** (passes on today's code; must never move)
...
// Pre-change record for the outline fixes. What every channel requests in the outline Data block,
// the shared tiles, Kenect's variant, the breakdown rows, and today's normalized post (no author key).
...
test("today's normalized Instagram post has no author key", () => {
  const raw = { id: 1, source: 'INSTAGRAM', type: 'IMAGE', source_created_at: '2026-08-02T10:00:00Z', media_group: null,
    instagram: { caption: 'x', url: 'u', sum_total_engagements: 3, views: 30, engagement: 0.01 }, instagram_user: { handle: 'brand_handle' } }
  const post = normalizePost(raw as never, 'INSTAGRAM')
  expect('author' in post).toBe(false)
  expect(post.metrics).toMatchSnapshot()
})
```

- [ ] **Step 2: Run it twice; confirm PASS and a stable snapshot.** Also run the existing pins: `npx vitest run lib/organic-social/outline-fixes-parity.test.ts components/report-sections/organic-social/parts/top-content-v2.golden.test.tsx lib/organic-social/fetch-top-content.test.ts` (all PASS).
...
- [ ] **Step 1: Failing tests.** In `outline-layout.test.ts`: replace the `OUTLINE_PENDING_Q6` test (lines 38-58 today) and its import with:
...
test('a media row or an alias row never enters the tiles request', () => {
  for (const c of ['INSTAGRAM', 'FACEBOOK', 'LINKEDIN'] as const) {
    expect(outlineSpecsFor(c).some((s) => s.key === 'videoViews' && c === 'INSTAGRAM')).toBe(false)
  }
})
```
...
**Interfaces produced:** `buildMediaKpis(channel: DashChannel, data: Record<string, { metrics?: Record<string, { ALL_CHANNELS?: TotalMetric }> }> | undefined, brandId: number, specs: MediaKpiSpec[]): Record<string, HeadlineKpi>`; `getOutlineMediaKpis(slug, dateRange, compareRange, channel, specs)` (React `cache`).
...
import type { MediaKpiSpec } from './outline-layout'
...
export const getOutlineMediaKpis = cache(async (
  slug: string, dateRange: string, compareRange: string | null, channel: DashChannel, specs: MediaKpiSpec[],
  ...
  const res = await client.getReportsData<{ ALL_CHANNELS?: TotalMetric }>({
    brandId, channels: [channel], reportType: 'MULTI_METRIC_MEDIA_TYPE',
...
| 3 | input boundary | no reels in the window (media type absent) | `buildMediaKpis` | fix: zero, no arrow; test |
| 4 | bounds | request count | one extra request per Instagram standard tab | accept: cached per request, only when the row shows |
...
- [ ] **Step 1: Failing tests** in `outline-parts.test.tsx` (mock `@/lib/organic-social/outline-media` with `getOutlineMediaKpis: vi.fn()` beside the existing mocks):
  ...
  - when `getOutlineMediaKpis` rejects, the other tiles still render and Video Views is a blank tile with the flag `Could not load from Dash`, and `console.error` is called once with `[organic-social] Views on Reels failed slug=<slug> channel=INSTAGRAM`;
...
export async function OutlineDataSection({ ctx, channel, rows }: { ctx: OrganicSocialCtx; channel: DashChannel; rows: readonly OutlineRow[] }) {
  ...
  const [r, m] = await Promise.all([
    ...
    media.length ? safe(getOutlineMediaKpis(ctx.clientSlug, ctx.dateRange, ctx.compareRange, channel, media)) : Promise.resolve({ data: {} }),
  ])
  if (!r.data) return <Fallback kind={r.error!} />
  if (!m.data) console.error(`[organic-social] Views on Reels failed slug=${ctx.clientSlug} channel=${channel}`)
...
**Files:** Create `lib/organic-social/post-author.ts`; modify `lib/organic-social/content-types.ts` (the `TopContentPost` interface only, after `sourceType`), `lib/organic-social/top-content.ts` (signature and the two `content.map` lines only); Test `lib/organic-social/post-author.test.ts`, extend `fetch-top-content.test.ts`

- [ ] **Step 1: Failing tests:** `authorOf` returns the lowercased handle without `@` from `instagram_user.handle`, else `.username`, else `null`; `null` for non-Instagram channels. `fetchTopContent(slug, range, 'INSTAGRAM')` returns posts with no `author` key (deep-equal to today's output in the existing test fixtures); with `{ withAuthor: true }` every Instagram post (owned and UGC) has `author` set from its raw record.
- [ ] **Step 2: Run, see FAIL.**
...
and the two lines become `content.map((p) => withAuthorIf(normalizePost(p, ch), p, ch, opts.withAuthor))` and `content.map((p) => withAuthorIf(normalizePost(p, 'INSTAGRAM'), p, 'INSTAGRAM', opts.withAuthor))`. `toPayload` (unchanged) stores `author` only when present, so no existing caller's snapshot rows change.
...
### Task 5: `top-content@3` (5 per page, collab by author, views-basis Instagram rate)

**Files:** Create `lib/organic-social/outline-top-content.ts`, `components/report-sections/organic-social/parts/top-content-outline.tsx`; modify `parts/top-content.tsx` (add `export` to `groupPostsByPlatform` and `loadDesignations` only), `parts/registry.ts`; Test `lib/organic-social/outline-top-content.test.ts`, `parts/top-content-outline.test.tsx`

- [ ] **Step 1: Failing tests** (pure, `outline-top-content.test.ts`):

```ts
import { expect, test } from 'vitest'
import { parseOwnHandles, partitionByAuthor, postsPerPage, withViewsBasisRate } from './outline-top-content'
...
test('posts per page: the pin threshold, default 5, whole numbers 1 to 50 only', () => {
  expect([postsPerPage(undefined), postsPerPage(6), postsPerPage(0), postsPerPage(51), postsPerPage(4.5)]).toEqual([5, 6, 5, 5, 5])
})
```

Part test (`top-content-outline.test.tsx`, mocks as in `top-content-v2.golden.test.tsx`): `top-content@3` is registered unpublished; it calls `fetchTopContentFrozen` with an injected `fetchLive` that asks `fetchTopContent` for authors; passes `pageSize` 5 (or the pin threshold) to `SortableTopContent`; an author post lands in the Influencer section; Instagram cards carry the views-basis rate. And a registry test: `top-content@1` and `@2` are the same objects as before (`toBe`).
- [ ] **Step 2: Run, see FAIL.**
...
export function postsPerPage(threshold: number | undefined): number {
...
import { parseOwnHandles, partitionByAuthor, postsPerPage, withViewsBasisRate } from '@/lib/organic-social/outline-top-content'
...
export async function TopContentOutlineSection({ ctx, perPage }: { ctx: OrganicSocialCtx; perPage: number }) {
  ...
  return (
    <section className="space-y-6">
      ...
      <SortableTopContent owned={groupPostsByPlatform(owned, channel)} influencer={groupPostsByPlatform(influencer, channel)}
        clientSlug={clientSlug} canEdit={canSetDesignation(role)} pageSize={perPage} />
    </section>
  )
}
...
export const topContentV3: PartImpl<OrganicSocialCtx> = {
  ...
  render: (ctx, resolved) => (
    <Suspense fallback={<TopContentSkeleton />}>
      <TopContentOutlineSection ctx={ctx} perPage={postsPerPage(resolved.threshold)} />
    </Suspense>
  ),
}
```
...
| 5 | bounds | a bad threshold | `postsPerPage` | fix: default 5, 1 to 50; test |
...
- [ ] Zero conflicts: `git merge-tree --write-tree` of this branch with each of 247, 250, 252, 253, 254, 256 (255 is its base), then all merged in two orders on a scratch worktree off `origin/dev`, same tree, tests, tsc and `check:rsc` green there (TikTok's Video Views row is exercised only once 247 is in).
...
1. After 255 and this are on staging: pin `top-content@3` for the three clients in `report_section_config.organic-social` (a `versions` override), and set `dash_social_config.ownHandles.instagram` for each (values read from Dash, never committed). Pre-change snapshot, dry run, then write; drift check after.
2. Decide Decision 3 (re-freeze August Top Content on staging) together with the lock-every-number rollout.
