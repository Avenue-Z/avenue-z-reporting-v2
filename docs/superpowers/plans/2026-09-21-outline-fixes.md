...
2. **Collab rule replaces the #ad suggestion for pinned clients**, only when the author and the client's own handle are both known; otherwise it falls back to today's #ad rule. A stored designation (a team member's manual choice) still wins, as today. Evidence: Jasmine on the Renaissance dashboard Q&A, "Manual judgement ... the fact that they're mentioning us and inviting us on a collaboration is a sign"; DFA Q8, "a separate section for collab posts towards the bottom"; the rule matches APFM's August deck 9 of 9 with 0 extras (today's #ad rule gets 7 of 9).
3. **Any Top Content window frozen before `top-content@3` is pinned has no authors** (snapshots are keyed by client, channel and window only, `frozen.ts:54`, and a frozen window never calls the live fetch, `frozen.ts:67`), so the author rule would fall back to #ad for it with no signal. So the rollout re-freezes the three clients' closed Instagram windows in the same go as the pin, right after it, and @3 logs a warning whenever it has an own handle but no Instagram post carries an author. Technical; moot once lock every number (PR 256) ships.
4. **Instagram only** for the author rule and the views-basis card rate. Evidence: collab posts are an Instagram feature; the card rate follows Jasmine's "engagement rate should divide by views not followers so just follow the deck" (D25), and Facebook's and LinkedIn's card rates already match their tile basis; TikTok's is set by PR 247.
