...
| `lib/salesforce/pipeline.test.ts` | Vitest, mocks `@/lib/salesforce/base` |
...
| `vitest.config.ts` | two new `lib/salesforce/*.test.ts` entries in the pinned include list |
...
Expected: a new file `drizzle/0021_<words>.sql` containing exactly `ALTER TABLE "clients" ADD COLUMN "salesforce_config" jsonb;`, plus a snapshot and a journal entry. Verify with `grep -l salesforce_config drizzle/*.sql`. **Do not run `db:migrate`.**
...
- Migration: `drizzle/0021_<words>.sql` (substitute the generated name)
...
- Produces: `PipelineData`, `transformPipeline(rows, cmpRows)`, `getSalesforcePipeline(slug)`, `transformByOwner(rows)`
...
In `vitest.config.ts`, the `include` array is a pinned allowlist. Add these two entries near the existing `lib/meta/kpis.test.ts` line:
...
(`contacts.test.ts` does not exist yet; vitest tolerates a missing include entry. Task 4 creates it.)
...
covers 1,848 renewals carrying zero dollars.
...
Expected: clean, no conflicts (Half A touches no file PR 207 touched). If conflicts appear, stop and escalate; the branches were designed disjoint.

- [ ] **Step 3: Verify the merged state**

Run: `npx tsc --noEmit && npm test 2>&1 | tail -3`
Expected: tsc clean; the suite green including PR 207's 30 tests and Half A's 14.
...
which is blank on nearly every record in their CRM. A truncation note
appears when the owner query hit its row cap, since the API gives no
other signal."
```
