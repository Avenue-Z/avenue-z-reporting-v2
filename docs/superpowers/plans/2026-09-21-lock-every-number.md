...
3. **The compare baseline is the prior month's locked value** when both months are locked (evidence: the team compares against what it reported before, not what Dash says today. APFM's August deck compares August's views with the June number the team reported, which no longer matches what Dash returns for June now, DFA Q3) (applied when the answer is read, so capture order never matters): September's "vs August" uses August's locked number, so a client dividing the two locked reports gets the arrow shown. When the prior month is not locked (the first reporting month's July, or previous-year comparisons before a year of history), Dash's own compare value is used.
