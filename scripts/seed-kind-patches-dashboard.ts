...
// Shopify (direct ShopifyQL via Admin API) leaf, optionally filtered to one shipping
// country. Bypasses the Supermetrics SHP source, whose queries hang for this store.
const shp = (metric: string, country?: string): LeafBinding => ({
  source: 'shopify',
  query: `FROM sales SHOW ${metric}${country ? ` WHERE shipping_country = '${country}'` : ''}`,
})
// Per-country net revenue formula: total_sales − taxes.
const netRevenue = (country: string) => ({
  source: 'formula' as const, expr: '@t - @tax',
  operands: { t: { kind: 'metric' as const, leaf: shp('total_sales', country) }, tax: { kind: 'metric' as const, leaf: shp('taxes', country) } },
})
