...
/** A note's picks as posts: the ids found among this day's posts in Dash's answer, in pick order; the
 *  rest are skipped. The client's card and the draft preview both resolve picks this way. */
const resolvePicks = (ids: number[], dayPosts: TopContentPost[]) =>
  ids.map((id) => dayPosts.find((p) => p.id === id)).filter((p): p is TopContentPost => !!p)

function attach(a: Annotation, dn: DayNote | undefined, dayPosts: TopContentPost[]): Annotation {
  if (!dn) return a
  const picks = resolvePicks(dn.approved?.postIds ?? [], dayPosts)
  // Approve sends the draft's picks, so an editor also gets the pictures the card will show once the
  // draft is approved, under the draft line: its picks Dash still returns, else the day's top post
  // (`a.post`, what the card falls back to), else none (Paul's review of #273, C3, and his second, R2).
  const draft = dn.editor?.draft
  const draftPicks = draft ? resolvePicks(draft.postIds, dayPosts) : []
  const editor = dn.editor && draft
    ? { ...dn.editor, draftThumbs: draftPicks.length > 0 ? draftPicks.map(thumbOf) : a.post ? [thumbOf(a.post)] : [] }
    : dn.editor
